...
export default class VisitedProfileProjectsScreen extends React.Component {
    ...
    state = { userOnline:true,normalUser:false,
 ...
 //    this.setState({ data: childSnapshot.val() })
 //    this.state.ArrayOfData = Object.keys(this.state.data)
 ...
 //  })
  	...
  	// 	console.log(items)
  	// 	this.setState({
  	// 		itemsDataSource:this.state.itemsDataSource.cloneWithRows(items)
  	// 	})
  	// })
  }
    ...
    // ...
  	...
  	_renderItem = ({ item }) => (
		            ...
		            <ListItem
			              ...
			              subtitle={
			              		<View>
						            <Text style={styles.bookmarkSubtitle}>{`${item.subtitle}`}</Text>
						            <Text style={styles.bookmarkDate}>{`${item.date}`}</Text>
					          	</View>
					        }
			              
			              avatar={<Thumbnail square size={54} source={{uri:item.avatar_url}} />}
